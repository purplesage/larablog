...
{{-- <div>
  <img src="{{ asset('storage/' . $post->thumbnail) == null ? asset('storage/' . $post->thumbnail) : $post->thumbnail}}"
    alt="Blog Post illustration" class="rounded-xl">
</div> --}}

{{-- {{dd(asset('storage/' . $post->thumbnail))}} --}}
...
<article {{$attributes->merge(['class' => "transition-colors duration-300 hover:bg-gray-100 border border-black
  ...
  <div class="py-6 px-5">
    <div>
      <img src="{{$imgURL}}" alt="Blog Post illustration" class="rounded-xl">
    </div>
